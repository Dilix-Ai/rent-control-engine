...
- **Dual ESM + CommonJS build.** `dist/esm/` ships ES modules with
  `.js`-extension imports (Node-ESM compatible); `dist/cjs/` ships
  CommonJS. `package.json` `exports` field routes import vs. require
  to the right bundle. Broadens compatibility for plain-Node ESM
  consumers, Lambda/Cloud-Functions runtimes, older bundlers, and
  any caller that uses `require()`.
- `tsconfig.cjs.json` extends the main config with `module: CommonJS`.
- `dist/cjs/package.json` autogenerated marker (`{"type":"commonjs"}`)
  so Node treats `dist/cjs/*.js` as CommonJS even though the package
  root is `"type": "module"`.
